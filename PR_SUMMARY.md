...
9. **A full-screen hero made of CSS.** The headline is the artwork: it sits centred over a drifting aurora of
   three blurred colour fields, a faint grid and three thin orbit rings with two dots running along them.
   Everything is sized in `vmin` and `svh`, so the section is exactly one screen tall from a 320 px phone to a
   1920 px desktop, with no canvas, no image and no per-device layout. Measured at seven viewports: the hero
   matches the viewport height at each one, with no horizontal overflow and no overlap between the proof row
   and the scroll cue.
...
| Dictionary parity EN/DE/BG | `npm run check:i18n` | 12 namespaces, 1 403 strings, 0 errors |
